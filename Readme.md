...
## Отчёт и материалы по выполненной работе
- `docs/report.md` — сводный отчёт (описание модулей до/после, таблица улучшений, код/лог/скриншоты).
- `docs/01_infra_audit.md` → аудит Compose/инфраструктуры.
- `docs/02_rust_architecture.md` → слои rust_iss + AppState/DI.
- `docs/03_php_refactor.md` → сервисный слой Laravel.
- `docs/04_legacy_plan.md` → cron-обёртка Pascal и план миграции на Python CLI (`services/legacy-cli`).
- `docs/05_db_plan.md` → индексы, materialized view, batching.
- `docs/06_observability_tests.md` → единый формат ошибок, логгирование и сценарии тестов.

## Лёгкий запуск на слабом хосте
- `docker-compose.yml` стартует полный цикл (`db`, `rust_iss`, `php`, `nginx`, `legacy_cli`) и использует slim/alpine/distroless образы плюс жёсткие лимиты памяти.
...
- Легаси-генератор телеметрии переписан на **Python** (вместо Pascal) и работает в режиме cron по умолчанию.
